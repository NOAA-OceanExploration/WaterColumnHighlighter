@echo off
:: ======================================================================
:: CritterDetector Model Pre-download Script (Windows Version)
:: ======================================================================
::
...
echo ==========================================================================================
echo CritterDetector - Model Pre-download Script (Windows Version)
echo ==========================================================================================
...
echo.
echo [1/6] Downloading OWLv2 model (Base)...
echo     This is Google's Open-vocabulary detector with zero-shot capabilities for marine organisms.
python -c "from transformers import Owlv2Processor, Owlv2ForObjectDetection; processor = Owlv2Processor.from_pretrained('google/owlv2-base-patch16-ensemble'); model = Owlv2ForObjectDetection.from_pretrained('google/owlv2-base-patch16-ensemble'); print('OWLv2 Base model cached successfully.')" || echo Failed to cache OWLv2 Base model. Check your internet connection and transformers package version.

:: Download and cache the DETR model (ResNet-50)
echo.
echo [2/6] Downloading DETR model (ResNet-50)...
echo     This is Facebook's Detection Transformer for general object detection.
python -c "from transformers import DetrImageProcessor, DetrForObjectDetection; processor = DetrImageProcessor.from_pretrained('facebook/detr-resnet-50', revision='no_timm'); model = DetrForObjectDetection.from_pretrained('facebook/detr-resnet-50', revision='no_timm'); print('DETR ResNet-50 (no_timm) model cached successfully.')" || python -c "from transformers import DetrImageProcessor, DetrForObjectDetection; processor = DetrImageProcessor.from_pretrained('facebook/detr-resnet-50'); model = DetrForObjectDetection.from_pretrained('facebook/detr-resnet-50'); print('DETR ResNet-50 (default) model cached successfully.')" || echo Failed to cache DETR ResNet-50 model. Check your internet connection and transformers package version.

:: Pre-download YOLOv8 model (Ultralytics handles download, this just ensures it's cached)
echo.
echo [3/6] Pre-downloading YOLOv8 model (Nano)...
echo     This is Ultralytics' fast object detection model for general objects.
python -c "from ultralytics import YOLO; model = YOLO('yolov8n.pt'); print('YOLOv8 Nano model cached successfully.')" || echo Failed to cache YOLOv8 Nano model. Check ultralytics installation.

:: Download and cache the CLIP model
echo.
echo [4/6] Downloading CLIP model (Large Patch14)...
echo     This is OpenAI's vision-language model for patch classification.
python -c "from transformers import CLIPProcessor, CLIPModel; processor = CLIPProcessor.from_pretrained('openai/clip-vit-large-patch14'); model = CLIPModel.from_pretrained('openai/clip-vit-large-patch14'); print('CLIP ViT-L/14 model cached successfully.')" || echo Failed to cache CLIP model. Check your internet connection and transformers package version.

:: Download and cache the Grounding DINO model (Base)
echo.
echo [5/6] Downloading Grounding DINO model (Base)...
echo     This is an open-vocabulary detector that accepts text prompts for zero-shot detection.
python -c "from transformers import AutoProcessor, AutoModelForZeroShotObjectDetection; processor = AutoProcessor.from_pretrained('IDEA-Research/grounding-dino-base'); model = AutoModelForZeroShotObjectDetection.from_pretrained('IDEA-Research/grounding-dino-base'); print('Grounding DINO Base model cached successfully.')" || echo Failed to cache Grounding DINO Base model. Check your internet connection and transformers package version.

:: Pre-download YOLO-World model (Ultralytics handles download, this just ensures it's cached)
echo.
echo [6/6] Pre-downloading YOLO-World model (Large v2)...
echo     This is Ultralytics' YOLO-World model that combines YOLO architecture with zero-shot capabilities.
python -c "from ultralytics import YOLO; model = YOLO('yolov8l-worldv2.pt'); print('YOLO-World Large v2 model cached successfully.')" || echo Failed to cache YOLO-World Large v2 model. Check ultralytics installation.
